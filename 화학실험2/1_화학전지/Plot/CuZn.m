y = [1.104; 1.095; 1.070; 1.029]
x = [0; 0; -0.05916.*log10(0.1./0.01); -0.05916.*log10(0.1./0.001)]

fitresult = fit(x,y,'poly1');
y_fit = fitresult.p1 * x + fitresult.p2;
x_fit = [min(x).*1.4:(max(x)-min(x))/160:-min(x).*0.2]';
...
txt = {['E^{cell} = (' num2str(fitresult.p1) '\pm' num2str( Sd_p1 ) ') * 0.05916 log Q + (' num2str(fitresult.p2) '\pm' num2str( Sd_p2 ) ')'],...
    ['R^2 = ' num2str(Rsq)]};
text(x_fit(1,1).*0.95,y_fit_draw(1,1).*0.92,txt)
n = 1./fitresult.p1
