y = readvars('DATA.xlsx','Range','B2:B6');
x = readvars('DATA.xlsx','Range','A2:A6');
...
xposerr = ones(length(x),1).*0.01;
xnegerr = xposerr;
yposerr = ones(length(y),1);
ynegerr = yposerr;
...
xlabel('d [\mu m]')
ylabel('#of interference change [A.U]')
...
txt = {['y = (' sprintf('%.2e',fitresult.p1) '\pm' sprintf('%.2e', Sd_p1 ) ') * x + (' sprintf('%.2e',fitresult.p2) '\pm' sprintf('%.2e', Sd_p2 ) ')'],...
    ['R^2 = ' num2str(Rsq)]};
text(x_fit(1,1).*2.6,y_fit_draw(1,1).*(0.5),txt)
2./fitresult.p1
Sd_p1./fitresult.p1./fitresult.p1