y = readvars('DATA.xlsx','Range','E2:E8');
x = readvars('DATA.xlsx','Range','D2:D8');
...
title('150V Elctron Radius vs Current')
xlabel('R^{2} [m^{2}]')
ylabel('1/I^{2} [A^{-2}]')
...
txt = {['1/I^{2} = (' num2str(fitresult.p1) '\pm' num2str( Sd_p1 ) ') * R^{2} + (' num2str(fitresult.p2) '\pm' num2str( Sd_p2 ) ')'],...
    ['R^2 = ' num2str(Rsq)]};
text(x_fit(10,1),y_fit_draw(215,1),txt)

e_m = 2.*fitresult.p1.*150/(7.8*7.8*1E-8)
error = 2.*Sd_p1.*150/(7.8*7.8*1E-8)